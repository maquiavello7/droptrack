...
# ══════════════════════════════════════════════
    ...
    if mod == "📣 Monitoreo campañas":
        ...
        def parse_cop_input(text):
            ...
            if '.' in s and ',' in s:
                if s.rindex(',') > s.rindex('.'):
                    # 82.246,54 → colombiano
                    s = s.replace('.', '').replace(',', '.')
                else:
                    # 82,246.54 → americano
                    s = s.replace(',', '')
            elif ',' in s and '.' not in s:
                # 82246,54 → coma como decimal
                s = s.replace(',', '.')
            elif '.' in s:
                parts = s.split('.')
                ...
                if len(parts) > 2:
                    s = s.replace('.', '')
                # Un punto con 3 dígitos después = miles (82.246, 150.000)
                elif len(parts) == 2 and len(parts[1]) == 3:
                    s = s.replace('.', '')
                # Else: decimal normal (82.5)
            try:
        ...
        def fmt_parsed(val):
            """Muestra el valor parseado para confirmación."""
            if val > 0:
