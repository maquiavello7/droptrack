...
def save_to_storage():
    ...
    try:
        ...
        set_local_storage(STORAGE_KEY, json.dumps(data, ensure_ascii=False, default=str),
                          component_key='save_storage')
    except: pass

def load_from_storage():
    """Restaura datos desde localStorage al iniciar.
    streamlit-js-eval es asíncrono: devuelve None en el primer render.
    Se reintenta hasta 3 veces antes de marcar como cargado."""
    if not HAS_JS: return
    if st.session_state.get('_storage_loaded', False): return

    # Contador de intentos
    attempts = st.session_state.get('_storage_attempts', 0)
    if attempts >= 3:
    ...
    try:
        ...
        if raw and isinstance(raw, str) and raw.strip().startswith('{'):
            ...
            # ✅ Solo marcar como cargado cuando realmente se cargaron datos
            st.session_state._storage_loaded = True
        else:
            # No llegó el dato aún — incrementar intento y forzar rerun
            st.session_state['_storage_attempts'] = attempts + 1
            st.rerun()
    except Exception:
        st.session_state['_storage_attempts'] = attempts + 1
        if attempts >= 2:
