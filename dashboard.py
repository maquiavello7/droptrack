            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                try:
                    ...
                    for cat,g in dfc2.groupby('__cat'):
                        # Usar despachados como denominador (no dTot) para coherencia
                        # con la dona que solo muestra guías despachadas
    ...
    if mod == "📣 Monitoreo campañas":
        ...
        with st.expander("➕ Agregar alerta"):
            ...
            if st.button("Guardar alerta"):
                st.session_state.alertas.append({'kpi':ak,'cond':ac,'val':av,'desc':ad}); save_to_storage(); st.success("✅"); st.rerun()
        ...
        with st.expander("➕ Nueva tarea"):
            ...
            if st.button("Guardar") and tt:
                st.session_state.tareas.append({'tipo':tp,'titulo':tt,'fecha':str(tf),'nota':tn,'done':False}); save_to_storage(); st.success("✅"); st.rerun()
