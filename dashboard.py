...
> Dropi → Cartera → Historial → Exportar""")
        ...
        if dp_raw_det is not None and dd_raw_det is not None:
            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                pendPorFechaLite_s={}
                try:
                    # Pendientes = todo lo que NO tiene estado final
                    # Estado final = SOLO ENTREGADO o DEVOLUCION
                    # Usar FINANCIERO cuando esté disponible (1 fila/orden, sin perder
                    # órdenes CANCELADO/RECHAZADO con NÚMERO GUIA = NaN en el dedup)
                    if dd_fin_raw is not None:
                        _pf_src = dd_fin_raw.copy()
                        _pf_src.columns = [c.strip() for c in _pf_src.columns]
                        _pf_est_col = find_col(_pf_src, ['ESTATUS'])
                        _pf_fec_col = find_col(_pf_src, ['FECHA'])
                        if _pf_est_col and _pf_fec_col:
                            _pf_src['_est_n'] = _pf_src[_pf_est_col].apply(lambda s: norm_estado(str(s)))
                            _mask_pf = (_pf_src['_est_n'] != 'ENTREGADO') & (_pf_src['_est_n'] != 'DEVOLUCION')
                            dfpf = _pf_src[_mask_pf].copy()
                            dfpf['_fecha_dt'] = dfpf[_pf_fec_col].apply(lambda x: parse_dt_dropi(str(x)[:10]))
                            # Mapear columnas para drill-down
                            for std, cands in [('guia',['NÚMERO GUIA','NUMERO GUIA']),
                                               ('nombre',['NOMBRE CLIENTE']),('telefono',['TELÉFONO','TELEFONO']),
                                               ('ciudad',['CIUDAD DESTINO']),('transportadora',['TRANSPORTADORA']),
                                               ('estatus',['ESTATUS']),('novedad',['NOVEDAD']),
                                               ('vendedor',['VENDEDOR']),('valor',['VALOR DE COMPRA EN PRODUCTOS']),
                                               ('flete',['PRECIO FLETE'])]:
                                c = find_col(dfpf, cands)
                                if c: dfpf[std] = dfpf[c].astype(str).str.strip()
                        else:
                            dfpf = d_filt[~d_filt['estatus'].apply(es_e) & ~d_filt['estatus'].apply(es_d)].copy()
                    else:
                        dfpf = d_filt[~d_filt['estatus'].apply(es_e) & ~d_filt['estatus'].apply(es_d)].copy()
    ...
    # ══ FINANCIERO ══
        ...
        # Logístico: TOTAL DE LA ORDEN=recaudo, GANANCIA, PRECIO PROVEEDOR X CANTIDAD=costo
        # Financiero: PRECIO FLETE=flete (sin duplicados), COSTO DEVOLUCION FLETE
        # ⚠️ NO usar VALOR FACTURADO del financiero (es = PRECIO FLETE, no el valor de venta)
        # ⚠️ NO usar VALOR DE COMPRA EN PRODUCTOS del financiero (es = TOTAL DE LA ORDEN)
