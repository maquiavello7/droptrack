...
> Dropi → Cartera → Historial → Exportar""")
        ...
        if dp_raw_det is not None and dd_raw_det is not None:
            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                pendPorFechaLite_s={}
                try:
                    # Pendientes = despachados sin estado final (es_s)
                    # Excluye CANCELADO/RECHAZADO — solo guías que están en tránsito
                    dfpf=d_filt[d_filt['estatus'].apply(es_s)].copy()
                    # Usar FECHA de la orden (no FECHA GUIA GENERADA que puede ser posterior)
                    dcol='_fecha_dt'
