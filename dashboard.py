...
> Dropi → Cartera → Historial → Exportar""")
        ...
        if dp_raw_det is not None and dd_raw_det is not None:
            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                pendPorFechaLite_s={}
                try:
                    # Pendientes por fecha — leer directo del logístico RAW
                    # Dedup solo por NÚMERO GUIA (como lo hace el pivot de Excel)
                    # NO depender de d_filt (que pasa por groupby de 6 columnas)
                    _pf_guia_col = find_col(dd_raw_det, ['NÚMERO GUIA','NUMERO GUIA'])
                    _pf_est_col  = find_col(dd_raw_det, ['ESTATUS'])
                    _pf_fec_col  = find_col(dd_raw_det, ['FECHA'])
                    if _pf_est_col and _pf_fec_col:
                        ...
                        # es_s = despachado sin estado final (excluye CANCELADO/RECHAZADO)
                        ...
                        # Parsear FECHA (columna de la orden, formato DD-MM-YYYY)
                        dfpf['_fecha_dt'] = dfpf[_pf_fec_col].apply(lambda x: parse_dt_dropi(str(x)[:10]))
                        # Aplicar filtro de rango del informe
