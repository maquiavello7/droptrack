...
> Dropi → Cartera → Historial → Exportar""")
        ...
        if dp_raw_det is not None and dd_raw_det is not None:
            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                pendPorFecha_s={}
                pendPorFechaLite_s={}
                _pf_debug = {'step': 'start'}
                try:
                    _pf_debug['step'] = 'find_cols'
                    _pf_guia_col = None
                    _pf_est_col = None
                    _pf_fec_col = None
                    # Buscar columnas en el raw
                    for c in dd_raw_det.columns:
                        ...
                        if cu == 'FECHA' and not _pf_fec_col:
                            _pf_fec_col = c
                    _pf_debug['est_col'] = _pf_est_col
                    _pf_debug['fec_col'] = _pf_fec_col
                    _pf_debug['guia_col'] = _pf_guia_col
                    _pf_debug['raw_cols'] = [str(c) for c in dd_raw_det.columns[:10]]

                    if _pf_est_col and _pf_fec_col:
                        _pf_debug['step'] = 'dedup'
                        _pf = dd_raw_det.copy()
                        if _pf_guia_col:
                            _pf = _pf.drop_duplicates(subset=_pf_guia_col, keep='first')
                        _pf_debug['after_dedup'] = len(_pf)

                        _pf_debug['step'] = 'filter_es_s'
                        ...
                        _pf_debug['pendientes'] = len(dfpf)

                        _pf_debug['step'] = 'parse_fecha'
                        dfpf['_fecha_dt'] = dfpf[_pf_fec_col].apply(lambda x: parse_dt_dropi(str(x)[:10]))
                        _pf_debug['fecha_nat'] = int(dfpf['_fecha_dt'].isna().sum())
                        _pf_debug['fecha_ok'] = int(dfpf['_fecha_dt'].notna().sum())

                        if dfpf['_fecha_dt'].notna().any():
                            _pf_debug['step'] = 'date_filter'
                            dfpf_dated = dfpf[(dfpf['_fecha_dt']>=dF)&(dfpf['_fecha_dt']<=dT)]
                            _pf_debug['in_range'] = len(dfpf_dated)
                            if len(dfpf_dated) > 0:
                                dfpf = dfpf_dated

                            _pf_debug['step'] = 'strftime'
                            dfpf['_fd'] = dfpf['_fecha_dt'].dt.strftime('%d/%m')
                            pendPorFecha_s = {k:int(v) for k,v in dfpf.groupby('_fd').size().items()}
                            _pf_debug['result_fechas'] = len(pendPorFecha_s)
                            _pf_debug['result_total'] = sum(pendPorFecha_s.values())
                        ...
                        _pf_debug['step'] = 'NO_COLS'
                except Exception as e_pf:
                    _pf_debug['error'] = f"{type(e_pf).__name__}: {e_pf}"
                    import traceback
                    _pf_debug['traceback'] = traceback.format_exc()[-500:]
    ...
    if tipo_inf == "📦 Informe Logístico":
        ...
        with r2r:
            ...
            st.markdown(f"""<div style='color:#f59e0b;font-size:11px;background:#1e293b;padding:8px 12px;border-radius:6px;margin-bottom:8px;line-height:1.6'>
            <b>DEBUG pendPorFecha</b>: {len(pendPorFecha)} fechas, sinFinal={snap.get('sinFinal',0)}<br>
            step={_dbg.get('step','?')}, cols={_dbg.get('raw_cols','?')}<br>
            est={_dbg.get('est_col','?')}, fec={_dbg.get('fec_col','?')}, guia={_dbg.get('guia_col','?')}<br>
            dedup={_dbg.get('after_dedup','?')}, pend={_dbg.get('pendientes','?')},
            fecha_ok={_dbg.get('fecha_ok','?')}, fecha_nat={_dbg.get('fecha_nat','?')},
            in_range={_dbg.get('in_range','?')}<br>
            result: {_dbg.get('result_fechas','?')} fechas, {_dbg.get('result_total','?')} total<br>
            {'<b style="color:#ef4444">ERROR: ' + _dbg.get('error','') + '</b><br><pre>' + _dbg.get('traceback','') + '</pre>' if _dbg.get('error') else '✅ Sin error'}
            </div>""", unsafe_allow_html=True)
