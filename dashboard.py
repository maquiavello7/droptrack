...
def now_co():  return datetime.now(TZ_CO)
def today_co(): return now_co().date()
    ...
    try:
        ...
        if re.match(r'\d{2}-\d{2}-\d{4}',f):
            ...
            if h and h.lower() not in ['nan','none','']:
                return datetime.strptime(f"{f[:10]} {h[:5]}", '%d-%m-%Y %H:%M')
            else:
                return datetime.strptime(f[:10], '%d-%m-%Y')
...
if '_storage_loaded' not in st.session_state: st.session_state._storage_loaded = False

# ══════════════════════════════════════════════
# PERSISTENCIA — localStorage del navegador
# ══════════════════════════════════════════════
STORAGE_KEY = 'droptrack_v3_data'

def save_to_storage():
    """Guarda informes, tareas, alertas y cortes en localStorage."""
    if not HAS_JS: return
    try:
        # Solo guardar datos serializables (sin DataFrames raw)
        informes_save = []
        for inf in st.session_state.informes:
            inf_copy = {k: v for k, v in inf.items() if k not in ('_raw',)}
            # Limpiar history de datos muy grandes
            if 'history' in inf_copy:
                history_clean = []
                for h in inf_copy['history']:
                    h_copy = {k: v for k, v in h.items()
                              if k not in ('ordenesLite',) and not isinstance(v, pd.DataFrame)}
                    history_clean.append(h_copy)
                inf_copy['history'] = history_clean
            informes_save.append(inf_copy)

        data = {
            'informes': informes_save,
            'tareas':   st.session_state.tareas,
            'alertas':  st.session_state.alertas,
            'cortes':   st.session_state.cortes,
            'saved_at': now_co().isoformat(),
        }
        json_str = json.dumps(data, ensure_ascii=False, default=str)
        # Usar key único para forzar re-render del componente JS
        _save_n = st.session_state.get('_save_counter', 0) + 1
        st.session_state['_save_counter'] = _save_n
        set_local_storage(STORAGE_KEY, json_str,
                          component_key=f'save_storage_{_save_n}')
    except Exception:
        pass

def load_from_storage():
    """Restaura datos desde localStorage al iniciar.
    get_local_storage es asíncrono: devuelve None en el primer render,
    y el valor real en el segundo. NO hacer st.rerun() — el componente
    dispara su propio rerun cuando el valor llega."""
    if not HAS_JS: return
    if st.session_state.get('_storage_loaded', False): return

    # Safety: máximo 10 intentos para evitar loop infinito
    _att = st.session_state.get('_storage_att', 0) + 1
    st.session_state['_storage_att'] = _att
    if _att > 10:
        st.session_state._storage_loaded = True
        return

    try:
        raw = get_local_storage(STORAGE_KEY, component_key='load_storage')
        if raw and isinstance(raw, str) and raw.strip().startswith('{'):
            data = json.loads(raw)
            if data.get('informes') and not st.session_state.informes:
                st.session_state.informes = data['informes']
            if data.get('tareas') and not st.session_state.tareas:
                st.session_state.tareas = data['tareas']
            if data.get('alertas') and not st.session_state.alertas:
                st.session_state.alertas = data['alertas']
            if data.get('cortes'):
                st.session_state.cortes = data['cortes']
            st.session_state._storage_loaded = True
        # Si raw es None → NO marcar como cargado
        # El componente JS disparará un rerun automático cuando tenga el valor
    except Exception:
        pass
...
# ── Guardar pendiente: si el sync anterior marcó _needs_save,
# guardar ahora (en este render el componente JS SÍ ejecuta porque
# no hay st.rerun() inmediato después)
if st.session_state.get('_needs_save', False):
    save_to_storage()
...
with st.sidebar:
    ...
    if st.session_state.get('_storage_loaded'):
        n_inf = len(st.session_state.informes)
        st.caption(f"💾 {n_inf} informe{'s' if n_inf != 1 else ''} en memoria")
    else:
        st.caption("⏳ Cargando datos...")
        if st.button("🔄 Cargar datos guardados", key="force_load", use_container_width=True):
            st.session_state._storage_loaded = False
            st.session_state['_storage_att'] = 0
            st.rerun()
...
# ══════════════════════════════════════════════
    ...
    if 'ci_desde' not in st.session_state:
        st.session_state['ci_desde'] = today_co().replace(day=1)
    if 'ci_hasta' not in st.session_state:
        st.session_state['ci_hasta'] = today_co()
    ...
    mes_det, anio_det = detectar_mes_anio(nombre) if nombre else (None, today_co().year)
...
> Dropi → Cartera → Historial → Exportar""")
        ...
        if dp_raw_det is not None and dd_raw_det is not None:
            ...
            if st.button("🔄 Sincronizar ahora", key=f"run_sync_{inf['id']}", use_container_width=True):
                ...
                # NO filtrar Pancake por fecha — el usuario ya exporta el archivo
                # para el período correcto. Re-filtrar recorta órdenes por bordes
                # de hora/fecha y da un totalOrd incorrecto.
                ...
                # ══════════════════════════════════════════════════════════
                # MAPA DE FUENTES DE DATOS — REGLAS DE ORO
                # ══════════════════════════════════════════════════════════
                #
                # ┌─────────────────────┬──────────────────────────────────────────────────────┐
                # │ FUENTE              │ QUÉ SE LEE DE AHÍ                                   │
                # ├─────────────────────┼──────────────────────────────────────────────────────┤
                # │ d_filt              │ Conteos de estados, pendPorFecha, gráficos           │
                # │ (logístico mapeado  │ • estatus → ESTATUS                                  │
                # │  + dedup por guía)  │ • _fecha_dt → FECHA (fecha ORDEN, NO guía generada)  │
                # │                     │ • _fecha_guia_dt → FECHA GUIA GENERADA (solo p/días) │
                # ├─────────────────────┼──────────────────────────────────────────────────────┤
                # │ raw_filt            │ Valores financieros que SE SUMAN por upsell:         │
                # │ (logístico raw      │ • TOTAL DE LA ORDEN → recaudo                        │
                # │  dedup por guía)    │ • GANANCIA → ganancia entregadas                     │
                # │                     │ • PRECIO PROVEEDOR X CANTIDAD → costo mercancía      │
                # ├─────────────────────┼──────────────────────────────────────────────────────┤
                # │ dd_fin_raw          │ Valores que NO deben sumarse por upsell:             │
                # │ (financiero, 1/ord) │ • PRECIO FLETE → flete entregadas (y por tipo envío) │
                # │                     │ • COSTO DEVOLUCION FLETE → costo devolución          │
                # │                     │ ⚠️ VALOR FACTURADO = es el FLETE, NO el valor venta  │
                # │                     │ ⚠️ VALOR DE COMPRA EN PRODUCTOS = TOTAL DE LA ORDEN  │
                # │                     │ ⚠️ TOTAL EN PRECIOS DE PROVEEDOR = COSTO PROVEEDOR   │
                # ├─────────────────────┼──────────────────────────────────────────────────────┤
                # │ dp_raw_det          │ totalOrd, cronograma (días entrega), util pendiente  │
                # │ (Pancake completo)  │ NO se filtra por fecha (usuario ya exporta el rango) │
                # ├─────────────────────┼──────────────────────────────────────────────────────┤
                # │ dc_raw_det          │ Clasificación cartera, flujo de caja, garantías      │
                # │ (Cartera Dropi)     │ • MONTO, DESCRIPCIÓN, FECHA                         │
                # └─────────────────────┴──────────────────────────────────────────────────────┘
                #
                # PENDIENTES POR FECHA:
                #   Fuente  → d_filt (logístico dedup)
                #   Filtro  → es_s() (despachado sin estado final; excluye CANCELADO/RECHAZADO)
                #   Fecha   → _fecha_dt (columna FECHA de la orden)
                #   ⚠️ NO usar _fecha_guia_dt (FECHA GUIA GENERADA puede ser días después)
                #
                # ══════════════════════════════════════════════════════════
                RAW_COLS = {
                    'estatus':       ['ESTATUS'],
                    ...
                    # ⚠️ SOLO nombres del LOGÍSTICO — NO mezclar con financiero
                    # En financiero: VALOR FACTURADO=flete, VALOR DE COMPRA EN PRODUCTOS=total orden
                    'valor':         ['TOTAL DE LA ORDEN'],
                    'ganancia':      ['GANANCIA'],
                    'costo_prod':    ['PRECIO PROVEEDOR X CANTIDAD'],
                    'flete':         ['PRECIO FLETE'],
                ...
                # SIEMPRE usar el logístico como base (tiene TOTAL DE LA ORDEN, GANANCIA,
                # PRECIO PROVEEDOR X CANTIDAD con nombres correctos).
                # El financiero tiene columnas con NOMBRES DISTINTOS que confunden el mapeo:
                #   VALOR FACTURADO = es el FLETE (no el valor de la orden)
                #   VALOR DE COMPRA EN PRODUCTOS = es el TOTAL DE LA ORDEN
                # Por eso raw_filt se construye SOLO desde logístico.
                # Flete y costo devolución se toman del financiero DESPUÉS (sin duplicados por upsell).
                raw_source = dd_raw_det
                if raw_source is not None:
                    st.info(f"💰 Calculando métricas desde Dropi Logístico ({len(raw_source):,} filas)"
                            + (f" + Financiero ({len(dd_fin_raw):,} filas)" if dd_fin_raw is not None else ""))
                ...
                def raw_flete_by_carrier():
                    """Flete promedio por transportadora — órdenes DESPACHADAS (excluye cancelados).
                    Usa FINANCIERO (1 fila/orden) cuando está disponible."""
                    result = {}
                    src = dd_fin_raw if dd_fin_raw is not None else None
                    if src is not None:
                        _fc_est = find_col(src, ['ESTATUS'])
                        _fc_fle = find_col(src, ['PRECIO FLETE'])
                        _fc_tra = find_col(src, ['TRANSPORTADORA'])
                        if _fc_est and _fc_fle and _fc_tra:
                            df_d = src[src[_fc_est].apply(lambda s: es_despachado(str(s)))]
                            for t, g in df_d.groupby(_fc_tra):
                                ts = str(t).strip()
                                if ts in ('nan','None',''): continue
                                vals = pd.to_numeric(g[_fc_fle], errors='coerce').dropna()
                                vals = vals[vals > 0]
                                result[ts] = round(float(vals.mean()), 2) if len(vals) > 0 else 0
                            return result
                    # Fallback: logístico raw_filt
                    if raw_filt.empty or '_r_transportadora' not in raw_filt.columns: return result
                    df_d = raw_filt[raw_filt['_r_estatus_str'].apply(es_despachado)]
                    for t, g in df_d.groupby('_r_transportadora'):
                        ts = str(t).strip()
                        if ts in ('nan','None',''): continue
                ...
                def raw_cdev_by_carrier():
                    """Costo devolución promedio por transportadora — solo DEVOLUCION con costo > 0.
                    Usa FINANCIERO (1 fila/orden) cuando está disponible."""
                    result = {}
                    src = dd_fin_raw if dd_fin_raw is not None else None
                    if src is not None:
                        _fc_est = find_col(src, ['ESTATUS'])
                        _fc_cdev = find_col(src, ['COSTO DEVOLUCION FLETE'])
                        _fc_tra = find_col(src, ['TRANSPORTADORA'])
                        if _fc_est and _fc_cdev and _fc_tra:
                            df_d = src[src[_fc_est].apply(lambda s: norm_estado(str(s))=='DEVOLUCION')]
                            df_d_real = df_d.copy()
                            df_d_real['_cv'] = pd.to_numeric(df_d_real[_fc_cdev], errors='coerce').fillna(0)
                            df_d_real = df_d_real[df_d_real['_cv'] > 0]
                            for t, g in df_d_real.groupby(_fc_tra):
                                ts = str(t).strip()
                                if ts in ('nan','None',''): continue
                                result[ts] = round(float(g['_cv'].mean()), 2) if len(g) > 0 else 0
                            return result
                    # Fallback: logístico raw_filt
                    if raw_filt.empty or '_r_transportadora' not in raw_filt.columns: return result
                ...
                # ── Conteos de estado: usar RAW dedup por guía (no d_filt del groupby) ──
                # El groupby de 6 columnas pierde filas cuando HORA o GUIA son NaN.
                # Dedup solo por guía produce el conteo correcto.
                _cnt_est = find_col(dd_raw_det, ['ESTATUS'])
                _cnt_guia = find_col(dd_raw_det, ['NÚMERO GUIA','NUMERO GUIA'])
                _cnt_fec = find_col(dd_raw_det, ['FECHA'])
                if _cnt_est and _cnt_guia:
                    _cnt_df = dd_raw_det.drop_duplicates(subset=_cnt_guia, keep='first')
                    # Aplicar filtro de fecha
                    if _cnt_fec:
                        _cnt_df = _cnt_df.copy()
                        _cnt_df['_cnt_dt'] = _cnt_df[_cnt_fec].apply(lambda x: parse_dt_dropi(str(x)[:10]))
                        _cnt_dated = _cnt_df[(_cnt_df['_cnt_dt']>=dF)&(_cnt_df['_cnt_dt']<=dT)]
                        if len(_cnt_dated) > 0:
                            _cnt_df = _cnt_dated
                    _cnt_est_col = _cnt_df[_cnt_est].astype(str).str.strip()
                    entregado  = int(_cnt_est_col.apply(es_e).sum())
                    devolucion = int(_cnt_est_col.apply(es_d).sum())
                    cancelados = int(_cnt_est_col.apply(es_cancelacion).sum())
                    sinFinal   = int(_cnt_est_col.apply(es_s).sum())
                    despachados= int(_cnt_est_col.apply(es_despachado).sum())
                    dTot       = len(_cnt_df)
                else:
                    # Fallback a d_filt si no hay raw
                ...
                # Logístico (raw_filt): recaudo, ganancia, costo mercancía
                # Financiero (dd_fin_raw): flete y costo devolución (1 fila/orden, sin duplicados)
                if not raw_filt.empty:
                    ...
                    # Flete y costo devolución: preferir FINANCIERO (sin duplicados por upsell)
                    if dd_fin_raw is not None:
                        _est_fin = find_col(dd_fin_raw, ['ESTATUS'])
                        _fle_fin = find_col(dd_fin_raw, ['PRECIO FLETE'])
                        _cdev_fin = find_col(dd_fin_raw, ['COSTO DEVOLUCION FLETE'])
                        if _est_fin and _fle_fin:
                            _mask_e_fin = dd_fin_raw[_est_fin].astype(str).str.upper().str.strip().apply(lambda s: norm_estado(s)=='ENTREGADO')
                            fle_ent = float(pd.to_numeric(dd_fin_raw[_mask_e_fin][_fle_fin], errors='coerce').fillna(0).sum())
                        else:
                            fle_ent = float(raw_filt[raw_filt['_r_es_e']==True]['_r_flete_n'].sum())
                        if _est_fin and _cdev_fin:
                            _mask_d_fin = dd_fin_raw[_est_fin].astype(str).str.upper().str.strip().apply(lambda s: norm_estado(s)=='DEVOLUCION')
                            cdev = float(pd.to_numeric(dd_fin_raw[_mask_d_fin][_cdev_fin], errors='coerce').fillna(0).sum())
                        else:
                ...
                snap = {
                    'timestamp': now_co().isoformat(),
                    '_v': 2,  # versión del código que generó este snap
                ...
                byEstadoCat_s = {}
                try:
                    # Usar raw dedup por guía (no d_filt del groupby que pierde filas)
                    _bc_guia = find_col(dd_raw_det, ['NÚMERO GUIA','NUMERO GUIA'])
                    _bc_est  = find_col(dd_raw_det, ['ESTATUS'])
                    _bc_val  = find_col(dd_raw_det, ['TOTAL DE LA ORDEN'])
                    if _bc_est and _bc_guia:
                        _bc_df = dd_raw_det.drop_duplicates(subset=_bc_guia, keep='first').copy()
                        if _cnt_fec:
                            _bc_df['_bc_dt'] = _bc_df[_cnt_fec].apply(lambda x: parse_dt_dropi(str(x)[:10]))
                            _bc_dated = _bc_df[(_bc_df['_bc_dt']>=dF)&(_bc_df['_bc_dt']<=dT)]
                            if len(_bc_dated) > 0:
                                _bc_df = _bc_dated
                        _bc_df['__cat'] = _bc_df[_bc_est].apply(cat_log)
                        _bc_df['__val'] = pd.to_numeric(_bc_df[_bc_val], errors='coerce').fillna(0) if _bc_val else 0
                        for cat, g in _bc_df.groupby('__cat'):
                            byEstadoCat_s[cat] = {
                                'count': len(g),
                                'valor': float(g['__val'].sum()),
                                'pct': round(pct(len(g), despachados), 2)
                            }
                    else:
                        dfc2=d_filt.copy(); dfc2['__cat']=dfc2['estatus'].apply(cat_log)
                        for cat,g in dfc2.groupby('__cat'):
                            byEstadoCat_s[cat]={'count':len(g),'valor':float(safe_col(g,'valor').sum()),'pct':round(pct(len(g),despachados),2)}
                except: pass
                    ...
                    try:
                        ...
                        if has_col(d_filt,'transportadora'):
                            ...
                            for t, g in dfc3.groupby('transportadora'):
                                ...
                                row['flete_avg'] = flete_by_carrier.get(_ns, 0)
                                row['cdev_avg']  = cdev_by_carrier.get(_ns, 0)
                                row['dias_avg']  = dias_by_carrier.get(_ns, 0)
                                byCarrierFull_s[_ns] = row
                ...
                pendPorFechaLite_s={}
                try:
                    # Buscar columnas directamente en el raw (sin find_col que depende de ns())
                    _pf_guia_col = _pf_est_col = _pf_fec_col = None
                    for c in dd_raw_det.columns:
                        cu = str(c).strip().upper()
                        if cu in ('NÚMERO GUIA','NUMERO GUIA') and not _pf_guia_col:
                            _pf_guia_col = c
                        if cu == 'ESTATUS' and not _pf_est_col:
                            _pf_est_col = c
                        if cu == 'FECHA' and not _pf_fec_col:
                            _pf_fec_col = c

                    if _pf_est_col and _pf_fec_col:
                        _pf = dd_raw_det.copy()
                        if _pf_guia_col:
                            _pf = _pf.drop_duplicates(subset=_pf_guia_col, keep='first')

                        _pf['_est_n'] = _pf[_pf_est_col].apply(lambda s: norm_estado(str(s)))
                        _pf_mask = _pf['_est_n'].apply(lambda n: n not in CANCELACIONES and n not in {'','NONE','NAN'} and n != 'ENTREGADO' and n != 'DEVOLUCION')
                        dfpf = _pf[_pf_mask].copy()

                        dfpf['_fecha_dt'] = dfpf[_pf_fec_col].apply(lambda x: parse_dt_dropi(str(x)[:10]))

                        if dfpf['_fecha_dt'].notna().any():
                            dfpf_dated = dfpf[(dfpf['_fecha_dt']>=dF)&(dfpf['_fecha_dt']<=dT)]
                            if len(dfpf_dated) > 0:
                                dfpf = dfpf_dated

                            dfpf['_fd'] = dfpf['_fecha_dt'].dt.strftime('%d/%m')
                            pendPorFecha_s = {k:int(v) for k,v in dfpf.groupby('_fd').size().items()}

                            # Mapear columnas para drill-down
                            _pf_map = {'guia':['NÚMERO GUIA','NUMERO GUIA'],'nombre':['NOMBRE CLIENTE'],
                                       'telefono':['TELÉFONO','TELEFONO'],'ciudad':['CIUDAD DESTINO'],
                                       'transportadora':['TRANSPORTADORA'],'estatus':['ESTATUS'],
                                       'novedad':['NOVEDAD'],'vendedor':['VENDEDOR'],
                                       'ult_mov':['ÚLTIMO MOVIMIENTO','ULTIMO MOVIMIENTO'],
                                       'fecha_umov':['FECHA DE ÚLTIMO MOVIMIENTO','FECHA DE ULTIMO MOVIMIENTO'],
                                       'valor':['TOTAL DE LA ORDEN'],'flete':['PRECIO FLETE']}
                            for std, cands in _pf_map.items():
                                c = find_col(dfpf, cands)
                                if c: dfpf[std] = dfpf[c].astype(str).str.strip()
                ...
                # ── Guardar versiones ligeras para fallback sin archivos ──
                # Dropi: columnas clave para Novedades y Seguimiento
                try:
                    _dl_cols_map = {
                        'ESTATUS':['ESTATUS'],'NÚMERO GUIA':['NÚMERO GUIA','NUMERO GUIA'],
                        'NOVEDAD':['NOVEDAD'],'SOLUCIÓN':['SOLUCIÓN','SOLUCION'],
                        'FUE SOLUCIONADA LA NOVEDAD':['FUE SOLUCIONADA LA NOVEDAD'],
                        'TRANSPORTADORA':['TRANSPORTADORA'],'CIUDAD DESTINO':['CIUDAD DESTINO'],
                        'DEPARTAMENTO DESTINO':['DEPARTAMENTO DESTINO'],
                        'NOMBRE CLIENTE':['NOMBRE CLIENTE'],'TELÉFONO':['TELÉFONO','TELEFONO'],
                        'TOTAL DE LA ORDEN':['TOTAL DE LA ORDEN'],'PRECIO FLETE':['PRECIO FLETE'],
                        'PRODUCTO':['PRODUCTO'],'VENDEDOR':['VENDEDOR'],
                        'ÚLTIMO MOVIMIENTO':['ÚLTIMO MOVIMIENTO','ULTIMO MOVIMIENTO'],
                        'FECHA DE ÚLTIMO MOVIMIENTO':['FECHA DE ÚLTIMO MOVIMIENTO','FECHA DE ULTIMO MOVIMIENTO'],
                        'HORA DE ÚLTIMO MOVIMIENTO':['HORA DE ÚLTIMO MOVIMIENTO','HORA DE ULTIMO MOVIMIENTO'],
                        'TIPO DE ENVIO':['TIPO DE ENVIO'],'FECHA':['FECHA'],
                    }
                    _dl_src = dd_raw_det.copy()
                    _dl_guia = find_col(_dl_src, ['NÚMERO GUIA','NUMERO GUIA'])
                    if _dl_guia:
                        _dl_src = _dl_src.drop_duplicates(subset=_dl_guia, keep='first')
                    _dl_df = pd.DataFrame()
                    for std_name, cands in _dl_cols_map.items():
                        c = find_col(_dl_src, cands)
                        _dl_df[std_name] = _dl_src[c].astype(str).str.strip() if c else ''
                    snap['_dropi_lite'] = _dl_df.head(600).to_dict('records')
                except Exception:
                    snap['_dropi_lite'] = []

                # Cartera: para Flujo de Caja
                try:
                    if dc_raw_det is not None:
                        _cl_src = dc_raw_det.copy()
                        _cl_src.columns = [c.strip() for c in _cl_src.columns]
                        _cl_cols = ['FECHA','TIPO','MONTO','DESCRIPCIÓN','DESCRIPCION','ORDEN ID','NUMERO DE GUIA','CONCEPTO DE RETIRO']
                        _cl_keep = [c for c in _cl_src.columns if c.upper() in [x.upper() for x in _cl_cols]]
                        snap['_cartera_lite'] = _cl_src[_cl_keep].head(800).to_dict('records')
                    else:
                        snap['_cartera_lite'] = []
                except Exception:
                    snap['_cartera_lite'] = []
                ...
                st.session_state._needs_save = True  # guardar en el próximo render
    ...
    # Avisar si los datos son de una versión vieja del código
    if snap and not snap.get('_v'):
        st.warning("⚠️ Estos datos fueron sincronizados con una versión anterior. "
                   "Re-carga los archivos y haz clic en **Sincronizar ahora** para actualizar.")
    ...
    if tipo_inf == "📦 Informe Logístico":
        ...
        with eo_l:
            ...
            fig_eo.add_annotation(text="% Confirmación",
                x=0.5,y=0.42,showarrow=False,font=dict(size=11,color='#94a3b8',family='Inter'))
            fig_eo.update_layout(**PLOT_BG,height=290,xaxis=AX,yaxis=AX,showlegend=False,
                title=dict(text=f"{snap['totalOrd']:,} órdenes totales  ·  {snap['despachados']:,} despachadas ({snap['pctConf']:.2f}%)",
            ...
            with mc1: st.markdown(f'<div class="big-kpi" style="text-align:center"><div style="font-size:22px;font-weight:800;color:#f1f5f9">{snap["totalOrd"]:,}</div><div style="font-size:10px;color:#64748b;margin:4px 0">&nbsp;</div><div style="font-size:10px;color:#64748b">total órdenes (Pancake)</div></div>', unsafe_allow_html=True)
        ...
        with r2r:
            ...
            n_pend_fecha = sum(v for v in pendPorFecha.values() if isinstance(v, (int, float)))
            if n_pend_fecha > 0:
                ...
                ℹ️ Tienes <strong>{n_pend_fecha:,} guías</strong> sin estado final. Las de más de 7 días tienen alto riesgo de devolución.</div>''', unsafe_allow_html=True)
        ...
        for geo_data, geo_col, geo_title, dl_key in [
            ...
            if not df_geo.empty:
                ...
                st.download_button(f"⬇️ CSV {geo_col.title()}",
                    ...
                    file_name=f"{geo_col.lower()}_{now_co().strftime('%Y%m%d')}.csv",
        ...
        if byCarrierFull:
            ...
            with pr1:
                ...
                st.caption("Calculado con guías despachadas.")
        ...
        if ords:
            ...
            st.download_button("⬇️ CSV logístico completo",
                ...
                file_name=f"logistico_{inf['title'].replace(' ','_')}_{now_co().strftime('%Y%m%d')}.csv",
    ...
    # ══ FINANCIERO ══
        ...
        if d_ is None and not snap:
            st.info("👆 Carga el archivo de Dropi y sincroniza el informe")
            st.stop()

        # ─── Datos base: desde archivos si están, sino desde snapshot ───
        if d_ is not None:
            ...
            # Sin archivos — usar snapshot
            n_ent   = snap.get('entregado', 0)
            n_dev   = snap.get('devolucion', 0)
            n_sinf  = snap.get('sinFinal', 0)
            n_desp  = snap.get('despachados', 0)
            df_ent = df_dev = df_sinf = df_desp = pd.DataFrame()
            st.markdown('''<div style="background:#1e3a5f;border:1px solid #3b82f6;border-radius:8px;
                padding:10px 14px;margin-bottom:14px;color:#93c5fd;font-size:12px">
                ℹ️ Mostrando datos de la última sincronización. Para actualizar, re-carga los archivos arriba y sincroniza.</div>''', unsafe_allow_html=True)
        ...
        # ══════════════════════════════════════════════════════════
        # FUENTE           │ COLUMNA EXACTA                │ MÉTRICA
        # ─────────────────┼───────────────────────────────┼────────────────
        # raw_log (logíst) │ TOTAL DE LA ORDEN (ENTREGADO) │ Recaudo de venta
        # raw_log (logíst) │ GANANCIA (ENTREGADO)          │ Ganancia
        # raw_log (logíst) │ PRECIO PROVEEDOR X CANT (ENT) │ Costo mercancía
        # raw_fin (financ) │ PRECIO FLETE (ENTREGADO)      │ Flete entregadas
        # raw_fin (financ) │ COSTO DEVOLUCION FLETE (DEV)  │ Costo devolución
        #
        # ⚠️ El financiero tiene columnas con NOMBRES ENGAÑOSOS:
        #    VALOR FACTURADO             = PRECIO FLETE (NO es valor de venta)
        #    VALOR DE COMPRA EN PRODUCTOS = TOTAL DE LA ORDEN
        #    TOTAL EN PRECIOS DE PROVEEDOR = PRECIO PROVEEDOR X CANTIDAD
        #    → NUNCA leer recaudo/ganancia/costo del financiero
        # ══════════════════════════════════════════════════════════
        ...
        if raw_log is not None:
            ...
            # Flete: preferir FINANCIERO (1 fila/orden, sin duplicados por upsell)
            fle_ent = _raw_sum(raw_fin, ['PRECIO FLETE'], es_e) if raw_fin is not None \
                      else _raw_sum(raw_log, ['PRECIO FLETE'], es_e)
        ...
        n_dev_real = n_dev
        n_dev_con_costo = 0
        src_cdev_display = raw_fin if raw_fin is not None else raw_log
        ...
        with ex1:
            ...
            st.download_button("⬇️ Descargar P&G (CSV)", data=pg_df.to_csv(index=False).encode('utf-8-sig'),
                file_name=f"PG_{inf['title'].replace(' ','_')}_{now_co().strftime('%Y%m%d')}.csv",
        ...
        with ex2:
            if dc_ is not None:
                ...
                st.download_button("⬇️ Descargar cartera CSV", data=dc_[cols_e].to_csv(index=False).encode('utf-8-sig'),
                    file_name=f"cartera_{now_co().strftime('%Y%m%d')}.csv",
...
# ══════════════════════════════════════════════
    ...
    if mod == "📣 Monitoreo campañas":
        ...
        # Fallback: reconstruir desde snapshot si no hay archivos raw
        if dropi_raw is None:
            _snap_n = inf_n['history'][-1] if inf_n.get('history') else None
            _dl = _snap_n.get('_dropi_lite', []) if _snap_n else []
            if _dl:
                dropi_raw = pd.DataFrame(_dl)
                st.markdown('''<div style="background:#1e3a5f;border:1px solid #3b82f6;border-radius:8px;
                    padding:10px 14px;margin-bottom:14px;color:#93c5fd;font-size:12px">
                    ℹ️ Mostrando datos de la última sincronización. Para actualizar, re-carga los archivos en el informe.</div>''', unsafe_allow_html=True)
        ...
        # Fallback: reconstruir desde snapshot si no hay archivos raw
        if dropi_s is None:
            _snap_s = inf_s['history'][-1] if inf_s.get('history') else None
            _dl_s = _snap_s.get('_dropi_lite', []) if _snap_s else []
            if _dl_s:
                dropi_s = pd.DataFrame(_dl_s)
                st.markdown('''<div style="background:#1e3a5f;border:1px solid #3b82f6;border-radius:8px;
                    padding:10px 14px;margin-bottom:14px;color:#93c5fd;font-size:12px">
                    ℹ️ Mostrando datos de la última sincronización. Para actualizar, re-carga los archivos en el informe.</div>''', unsafe_allow_html=True)
        ...
        st.download_button("⬇️ Descargar CSV",
            ...
            file_name=f"seguimiento_{lista_sel[:20].replace(' ','_')}_{now_co().strftime('%Y%m%d')}.csv",
        ...
        # Fallback: reconstruir desde snapshot si no hay archivo
        if cart_raw is None:
            _snap_fc = inf_fc['history'][-1] if inf_fc.get('history') else None
            _cl = _snap_fc.get('_cartera_lite', []) if _snap_fc else []
            if _cl:
                cart_raw = pd.DataFrame(_cl)
                st.markdown('''<div style="background:#1e3a5f;border:1px solid #3b82f6;border-radius:8px;
                    padding:10px 14px;margin-bottom:14px;color:#93c5fd;font-size:12px">
                    ℹ️ Mostrando datos de la última sincronización. Para actualizar, re-carga el archivo de Cartera en el informe.</div>''', unsafe_allow_html=True)
        ...
        # ── Bold: archivo de transacciones (pagos adelantados) ──
        st.markdown("---")
        bold_depositos = {}  # {day_int: deposit_amount}
        bold_file = st.file_uploader("💳 Bold — Reporte mensual de transacciones",
            type=['csv','xlsx','xls'], key=f"bold_{inf_fc['id']}",
            help="Columnas clave: FECHA, ESTADO ACTUAL, DEPOSITO EN CUENTA BOLD")
        if bold_file:
            try:
                bold_raw = load_file(bold_file)
                if bold_raw is not None:
                    # Detectar fila de encabezado (buscar 'ID TRANSACCION' o 'ESTADO ACTUAL')
                    _hdr_row = None
                    for i in range(min(5, len(bold_raw))):
                        row_vals = [str(v).strip().upper() for v in bold_raw.iloc[i].tolist()]
                        if 'ESTADO ACTUAL' in row_vals or 'ID TRANSACCION' in row_vals:
                            _hdr_row = i
                            break
                    if _hdr_row is not None:
                        bold_df = bold_raw.iloc[_hdr_row+1:].copy()
                        bold_df.columns = [str(v).strip() for v in bold_raw.iloc[_hdr_row].tolist()]
                    else:
                        bold_df = bold_raw.copy()

                    _b_est = find_col(bold_df, ['ESTADO ACTUAL','estado actual'])
                    _b_dep = find_col(bold_df, ['DEPOSITO EN CUENTA BOLD','deposito en cuenta bold'])
                    _b_fec = find_col(bold_df, ['FECHA','fecha'])

                    if _b_est and _b_dep and _b_fec:
                        # Solo cobros exitosos
                        bold_ok = bold_df[bold_df[_b_est].astype(str).str.upper().str.contains('EXITOSO', na=False)].copy()
                        bold_ok['_dep'] = pd.to_numeric(bold_ok[_b_dep], errors='coerce').fillna(0)
                        bold_ok['_fdt'] = pd.to_datetime(bold_ok[_b_fec], errors='coerce')
                        bold_ok['_dia'] = bold_ok['_fdt'].dt.day

                        for dia, grp in bold_ok.groupby('_dia'):
                            bold_depositos[int(dia)] = round(float(grp['_dep'].sum()))

                        total_bold = sum(bold_depositos.values())
                        st.success(f"✅ Bold: {len(bold_ok)} cobros exitosos · {fmt_cop(total_bold)}")
                    else:
                        st.warning("⚠️ No se encontraron las columnas ESTADO ACTUAL, DEPOSITO EN CUENTA BOLD o FECHA")
            except Exception as e_bold:
                st.warning(f"⚠️ Error leyendo Bold: {e_bold}")

        has_bold = len(bold_depositos) > 0
        ...
        st.markdown("---")
        if has_bold:
            st.markdown("**✏️ Consumo de publicidad por período** (Transferencias cargadas desde Bold)")
        else:
        ...
        # ── Manual inputs ──
        n_per = len(periodos)
        st.caption(f"Ingresa los valores para cada {'semana' if tipo_flujo == '📅 Semanal' else 'día'}. Los valores del diario se acumulan automáticamente en el semanal.")
        if not has_bold:
            st.caption("💡 Puedes escribir en formato colombiano: 82.246,54 o simplemente 82246")

        def parse_cop_input(text):
            """Convierte texto en formato colombiano a float redondeado.
            82.246,54 → 82247 | 82246 → 82246 | 1.500.000 → 1500000"""
            s = str(text).strip()
            if not s or s in ('0','0.0','0,0',''): return 0.0
            if '.' in s and ',' in s:
        ...
        def fmt_parsed(val):
            if val > 0:
                return f"<span style='font-size:10px;color:#10b981'>→ {fmt_cop(val)}</span>"
            return ""
        ...
        if tipo_flujo == "📆 Diario":
            ...
            for row_start in range(0, n_per, cols_per_row):
                ...
                for col_i, (per, lbl) in zip(row_cols, row_dias):
                    ...
                    day_n = per.day if hasattr(per, 'day') else 0
                    with col_i:
                        st.markdown(f"<div style='font-size:11px;font-weight:700;color:#0ea5e9;text-align:center'>{lbl}</div>", unsafe_allow_html=True)

                        # Transferencia: Bold automático o manual
                        if has_bold:
                            transf = bold_depositos.get(day_n, 0)
                            if transf > 0:
                                st.markdown(f"<div style='font-size:10px;color:#10b981;text-align:center'>T: {fmt_cop(transf)}</div>", unsafe_allow_html=True)
                            else:
                                st.markdown(f"<div style='font-size:10px;color:#64748b;text-align:center'>T: $0</div>", unsafe_allow_html=True)
                        else:
                            t_txt = st.text_input("T", value=str(int(saved['transf'])) if saved['transf'] else "",
                                key=f"fc_transf_{inf_fc['id']}_{per_str}", label_visibility="collapsed",
                                placeholder="Transf")
                            transf = parse_cop_input(t_txt)
                            if transf > 0: st.markdown(fmt_parsed(transf), unsafe_allow_html=True)

                        # Publicidad: siempre manual
                        p_txt = st.text_input("P", value=str(int(saved['pub'])) if saved['pub'] else "",
                            key=f"fc_pub_{inf_fc['id']}_{per_str}", label_visibility="collapsed",
                            placeholder="Pub")
                        pub = parse_cop_input(p_txt)
                        if pub > 0: st.markdown(fmt_parsed(pub), unsafe_allow_html=True)

                        st.session_state[manual_key][per_str] = {'transf': transf, 'pub': pub}
                        manual_vals[per] = {'transf': transf, 'pub': pub}

        else:
            # Semanal — aggregate from daily
            cols_inp = st.columns(5)
            for i, (per, lbl) in enumerate(zip(periodos, periodo_lbls)):
                # Sum daily values for this week
                if has_bold:
                    daily_transf = sum(
                        bold_depositos.get(d.day if hasattr(d,'day') else 0, 0)
                ...
                with cols_inp[i % 5]:
                    st.markdown(f"**{lbl}**")
                    if has_bold:
                        # Bold: show aggregated transfers (read-only)
                        st.markdown(f"<div style='font-size:11px;color:#64748b'>Transf: <strong style='color:#10b981'>{fmt_cop(daily_transf)}</strong> (Bold)</div>", unsafe_allow_html=True)
                        if daily_pub > 0:
                            ...
                            p_txt_w = st.text_input("Consumo publicidad ($)",
                                value="", key=f"fc_pub_w_{inf_fc['id']}_{i}",
                                placeholder="Ej: 150.000")
                            daily_pub = parse_cop_input(p_txt_w)
                            if daily_pub > 0: st.markdown(fmt_parsed(daily_pub), unsafe_allow_html=True)
                        manual_vals[per] = {'transf': daily_transf, 'pub': daily_pub}
                    elif daily_transf > 0 or daily_pub > 0:
                        ...
                        t_txt_w = st.text_input("Transf. pag. adelantados ($)",
                            value="", key=f"fc_transf_w_{inf_fc['id']}_{i}",
                            placeholder="Ej: 82.246,54")
                        transf = parse_cop_input(t_txt_w)
                        if transf > 0: st.markdown(fmt_parsed(transf), unsafe_allow_html=True)
                        p_txt_w = st.text_input("Consumo publicidad ($)",
                            value="", key=f"fc_pub_w_{inf_fc['id']}_{i}",
                            placeholder="Ej: 150.000")
                        pub = parse_cop_input(p_txt_w)
                        if pub > 0: st.markdown(fmt_parsed(pub), unsafe_allow_html=True)
                        manual_vals[per] = {'transf': transf, 'pub': pub}
        ...
        _transf_label = 'Transferencia de pagos adelantados (Bold)' if has_bold else 'Transferencia de pagos adelantados'
        FILAS = [
            ('entrada',      'Entrada (Ganancias) Dropi',          True),
            ('salida_flete', 'Salida (Fletes + Devoluciones + Nuevas órdenes)', False),
            ('transf',       _transf_label, False),
        ...
        with st.expander("➕ Agregar alerta"):
            ...
            if st.button("Guardar alerta"):
                st.session_state.alertas.append({'kpi':ak,'cond':ac,'val':av,'desc':ad}); st.session_state._needs_save = True; st.success("✅"); st.rerun()
        ...
        with st.expander("➕ Nueva tarea"):
            ...
            tf=st.date_input("Fecha",value=today_co())
            tn=st.text_area("Detalles")
            if st.button("Guardar") and tt:
                st.session_state.tareas.append({'tipo':tp,'titulo':tt,'fecha':str(tf),'nota':tn,'done':False}); st.session_state._needs_save = True; st.success("✅"); st.rerun()
